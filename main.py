...
def main():
    ...
    init_session_states()
    with st.container():
        st.write('<h1 style="color: #FF7F00; font-size: 72px; font-weight: bold;">Echo.</h1>', unsafe_allow_html=True)
    ...
    if st.session_state.user_authenticated:
        ...
        with create_tab:
            ...
            upload_mode = st.radio("Upload Mode", options=['Upload Your File', 'Record Your Voice'],horizontal=True)
            if upload_mode == 'Upload Your File':
                uploaded_file = st.file_uploader("Upload Audio File", type=['mp3', 'mp4', 'mpeg', 'mpga', 'm4a', 'wav', 'webm'])
                if uploaded_file is not None:
                    audio_bytes = uploaded_file.read()
                    st.audio(audio_bytes, format="audio/wav")

            if upload_mode == 'Record Your Voice':
                st.markdown(
                    """
                    <style>
                ...
                audio_bytes = audio_recorder(
                    ...
                    recording_color="#E67300",
                    neutral_color="#FF7F00",
                    icon_size="7x",
                    text="",
                )
            ...
            if st.button("Generate Transcript and Summary") and uploaded_file:
                ...
                with st.spinner('Processing...'):
                    ...
                    fact_check_prompt = PromptTemplate(
                        input_variables=['transcript', 'summary'],
                        template='''
                            Fact-check this transcript for factual or logical inaccuracies or inconsistencies
                            \nWrite a report on the factuality / logic of the transcript
                            \nTRANSCRIPT: {transcript}
                            \nTRANSCRIPT SUMMARY: {summary}
                    ...
                    insert_into_transcripts(file_name=os.path.basename(st.session_state.audio_file_path),
                    ...
                    insert_audio(file_path=st.session_state.audio_file_path, 
                                 transcript_id=get_transcript_id(file_name=os.path.basename(st.session_state.audio_file_path))
                    )
                    transcript_texts = [os.path.basename(st.session_state.audio_file_path),
            ...
            if st.session_state.audio_file_path:
                if st.session_state.transcript:
                    st.subheader(os.path.basename(st.session_state.audio_file_path))
